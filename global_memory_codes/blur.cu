...
int main(){
    ...
    blur<<<numBlocks,threadsPerBlock>>>(input_matrix,output_matrix,width,height);

    cudaDeviceSynchronize();

    cv::Mat flip(height,width,CV_8UC1);
    cudaMemcpy(flip.data,output_matrix,size,cudaMemcpyDeviceToHost);
