...
#include "binary_threshold_header.cuh"
...
int main(){
    ...
    binary_threshold<<<numBlocks,threadsPerBlock>>>(input_matrix,output_matrix,width,height,threshold_value,target_value);

    cudaDeviceSynchronize();

    cv::Mat binary(height,width,CV_8UC1);
    cudaMemcpy(binary.data,output_matrix,size,cudaMemcpyDeviceToHost);
