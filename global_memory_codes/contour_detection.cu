...
int main(){
    ...
    cudaMalloc((void**)&input_matrix,size_bgr);
    cudaMalloc((void**)&output_matrix,size);

    cudaMemcpy(input_matrix,resim.data,size_bgr,cudaMemcpyHostToDevice);
    cudaMemcpy(output_matrix,binary.data,size,cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(contour_detected.data,input_matrix,size_bgr,cudaMemcpyDeviceToHost);
